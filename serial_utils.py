...
mPduCmdDictSend = {'CMD_SET_CURRENTS': 30,
                   'CMD_SET_FANS': 31,
                   ...
                   'CMD_GET_TEMPS': 23,
                   'CMD_GET_FANS': 21}
mPduCmdDict2Rev = dict(zip(mPduCmdDictSend.values(), mPduCmdDictSend.keys()))
...
def build_message(head_frame, data_frame, tail_frame, codes, first_recursion=True):
    ...
    if isinstance(data_frame, str):  # str
        ...
        bytes_data = b''.join(
            [build_message(head_frame='', data_frame=i, tail_frame='', codes=codes, first_recursion=False) for i in
             data_frame])
    else:
...
def asu_pdu_parse_one_frame(message):
    """
    解析PDU返回的一帧数据
    :param message:
        bytes: b'U\xda\xf6(\x1c@\x14\xaeG@33c@\xb6'
...
def asu_pdu_build_one_frame(cmd, length, data_list):
    head = 'FEFE'
    total = 0
    strData = ''
...
def parse_multi_frame(message, head_length, tail_length, codes, debug=False):
    ...
    while current_idx < len(message):
        ...
        _, data, message_tail_frame = parse_one_frame(curr_message, total_length=message_length, codes=codes,
                                                      head_length=head_length, tail_length=tail_length)
        if message_tail_frame[2:].lower() != tail_frame.lower():  # check sum and real tail frame
            print("Parse error: tail frame %s doesn't equal to tail frame definition %s" % (
                message_tail_frame[2:], tail_frame))
            current_idx += 2  # go to next bytes and try again
            continue
