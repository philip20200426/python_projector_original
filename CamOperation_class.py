...
def Mono_numpy(data, nWidth, nHeight):
    ...
    # <class 'numpy.ndarray'> (2048, 3072, 1)
...
class CameraOperation:
    ...
    def Work_thread(self, winHandle):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                self.raw_numpy_img = Mono_numpy(self.buf_save_image, self.st_frame_info.nWidth,
    ...
    def Save_Bmp(self):
        ...
        file_path = 'asuFiles/interRefFiles/' + times.strftime("%Y-%m-%d %H:%M:%S").strip().replace(':', '_') + '.bmp'
        ...
        return ret
    def Save_Cal_Bmp(self, file_path):
        ...
        print('HK Take picture:', file_path)
        stSaveParam = MV_SAVE_IMAGE_TO_FILE_PARAM_EX()
        stSaveParam.enPixelType = self.st_frame_info.enPixelType  # ch:相机对应的像素格式 | en:Camera pixel type
        stSaveParam.nWidth = self.st_frame_info.nWidth  # ch:相机对应的宽 | en:Width
        stSaveParam.nHeight = self.st_frame_info.nHeight  # ch:相机对应的高 | en:Height
